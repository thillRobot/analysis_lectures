...
{\bf Lecture \LNUM - \secondtitle :} \vspc % ' topics' are beamer 'sections' - TWH

 \begin{itemize}
	\item Review\vspcc
	\item Example - The Trial Solution Method \vspcc
\end{itemize}
...
\frame{
 ...
 A solution to a problem that can be written in "closed form" in terms of known functions, constants, etc., is often called an {\bf analytic solution}. Note that this use of the word is completely different from its use in the terms analytic continuation, analytic function, etc. \vspc
...
A {\bf numerical solution} is an approximation to the solution of a mathematical equation, often used where analytical solutions are hard or impossible to find. All numerical solutions are approximations, some better than others, depending on the context of the problem and the numerical method used. \vspc
...
Remember our example from the previous lecture? If we add the external force to the model the equation is no longer separable.\vspcc
...
\scalebox{1.25}{$m\dot{v}+cv=F$} \vspcc
...
\underline{Step 1} - Find the {\bf complementary part} of the solution from the \vspace{1mm}\\ left hand side of the ODE alone (LHS=0). \vspcc  

\scalebox{1.25}{$m\dot{v}+cv=F \hspace{5mm}\rightarrow\hspace{5mm} m\dot{v}+cv=0$} \vspcc

We determined the solution to this part already. \vspc

\scalebox{1.25}{$v_{complementary}=v_c(t)=$}   \vspcc
...
\scalebox{1.25}{$v_{particular}=v_p(t)=$}   \vspc
...
\underline{Step 3} - Now combine the {\bf complementary} and {\bf particular} solutions through {\it superposition}. \vspcc  

\scalebox{1.25}{$v(t)=v_c(t)+v_p(t)=$} \vspcc

The ODE is first order and we have \underline{\hspace{10mm}} unknown. Coincidence?\vspcc

\scalebox{1.25}{$v(t)=$}   \vspcc
...
What does the solution look like this time?\vspcc

\scalebox{1.25}{$v(t)=(v_0-\frac{F}{c})e^{-\frac{c}{m}t}+\frac{F}{c}$} \vspcc
