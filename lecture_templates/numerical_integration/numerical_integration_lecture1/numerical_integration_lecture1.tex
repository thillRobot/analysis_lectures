...
You have probably been thinking about this idea for much longer than that. Consider riding in a car waiting to arrive somewhere...\vspace{10mm}\\
...
You may not have known it but you where {\bf integrating} when performing this calculation. You can math.\vspace{20mm}\\
...
Given a function describing the slope and an initial condition the solution can be approximated. 
This is Euler's method which is simply a rearrangement of the Taylor series used  \vspccc
...
for j=1:length(time)-1
    v_eulers(j+1)=v_eulers(j)+(f(time(j),v_eulers(j),m,c))*dt;  
end
